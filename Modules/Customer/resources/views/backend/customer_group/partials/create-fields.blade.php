...
{{ trans('customer::customer_group.labels.name') ,trans('customer::customer_group.labels.name').' *' }}
    ...
    {{ normalText("name","Name", $errors,null,['hide_label'=> true,"id" => "name", "class" => "form-control required".$className, "placeholder" => trans("customer::customer_group.labels.name")])}}
...
<div class="form-group">
    ...
    <span data-placement="right" data-toggle="tooltip" title="{!! trans('core::core.options.status.disable') !!}">
        <label class="switch">
            <input type="checkbox" name="is_default" class="status">
