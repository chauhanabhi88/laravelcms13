...
@section('title')
{{ trans("blog::blog_post.titles.create_blog_post") }}
@endsection
...
<div class="page-title-header row d-none d-sm-flex">
    <div class="page-header col-sm-12 d-flex pb-4 pt-2">
        <div class="col-sm-6">
            <h4 class="page-title">{{ trans("blog::blog_post.titles.create_blog_post") }}</h4>
        </div>
...
{{ formStart(null,"POST" ,'admin.blog_post.store' ,updateUrlParams(), ['id' => 'main_form','enctype'=>'multipart/form-data'])}}
            ...
            <div class="card-body">
                <div class="tab-content">
                        ...
                        <div class="card-body">
                            <div class="tab-content">
                                ...
                                <div class="tab-pane {{ App::getLocale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                                    @include('blog::backend.blog_post.partials.create-translatable-fields', ['lang' => $locale])
                                </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="tab-pane ctab-pane {{isset($languageOptions) && (count($languageOptions)) > 1 ? '' : 'active'}}" id="general-form">
                        ...
                        @endforeach
                        @endif
                        {{ normalSelect("blog_categories[]",trans("blog::blog_post.labels.blog_categories"),$errors, $blogCategories,null,  ["class" => "form-control select2 required","multiple" => "multiple"]) }}
                        {{ normalText("slug","blog::blog_post.labels.slug", $errors,null,["class" => "form-control required", "data-slug" => "target" ])}}
                        ...
                        {{ normalText("author","blog::blog_post.labels.author", $errors,null,["class" => "form-control required"])}}

                        {{ normalText("post_date","blog::blog_post.labels.post_date", $errors,null,["id"=>"blog_post_post_date","class" => "form-control required" ])}}
                        <div>
                        ...
                        <span data-placement="right" data-toggle="tooltip" title="{!! trans('core::core.options.status.disable') !!}">
                            <label class="switch">
                                <input type="checkbox" value="{{config('core.enabled')}}" class="status" name="is_featured">
                        ...
                        <span data-placement="right" data-toggle="tooltip" title="{!! trans('core::core.options.status.disable') !!}">
                            <label class="switch">
                                <input type="checkbox" value="{{config('core.enabled')}}" class="status" name="status">
                                <span class="slider round"></span>
                            </label>
                        </span>
                    </div>
