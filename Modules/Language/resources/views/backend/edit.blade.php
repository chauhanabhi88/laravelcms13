...
@section('title')
{{ trans("language::language.titles.edit_language") }} {{wordWrapper($language->title, true)}}
@endsection
...
<div class="page-title-header row d-none d-sm-flex">
    <div class="page-header col-sm-12 d-flex pb-4 pt-2">
        <div class="col-sm-6">
            <h4 class="page-title">{{ trans("language::language.titles.edit_language") }} {{wordWrapper($language->title, true)}}</h4>
        </div>
...
@section('content')
{{ formStart(null,"PUT" ,'admin.language.update' ,updateUrlParams([$language->id]), ['id' => 'main_form'])}}
    ...
    <div class="col-12 col-sm-6 col-lg-12">
        <div class="card card-info card-outline">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        {{ normalText("language[title]","language::language.labels.title", $errors,$language->title,["class" => "form-control required"])}}
                    </div>
                    <div class="col-md-6">
                        {{ normalText("language[locale]","language::language.labels.locale", $errors,$language->locale,["class" => "form-control required"])}}
                    </div>
                </div>
                {{ normalSelect("language[is_default]","language::language.labels.is_default",$errors, $yesNoOptions,$language->is_default,  ["class" => "form-control required"]) }}
                <div class="form-group">
                    ...
                    <span data-placement="right" data-toggle="tooltip" title="{!! $statusOptions[$language->status] !!}">
                        <label class="switch">
                            <input type="checkbox" name="language[status]" class="status" {{ ($language->status == 1) ? "checked" : ""}}>
