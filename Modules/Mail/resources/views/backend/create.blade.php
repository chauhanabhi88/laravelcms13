...
@section('title')
{{ trans("mail::mail.titles.create_mail") }}
@endsection
...
@endforeach
{{ formStart(null,"POST" ,'admin.mail.store' ,updateUrlParams(), ['id' => 'main_form','enctype'=>'multipart/form-data'])}}
                    ...
                    <div class="tab-pane fade show active" id="custom-tabs-three-page-info" page="tabpanel" aria-labelledby="custom-tabs-three-page-info-tab">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                {{ normalText("mail[name]","", $errors,null,['hide_label' => true ,"class" => "form-control required" ,"data-slug" => "source"])}}
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    @php $className = $errors->has('mail.slug') ? ' is-invalid' : ''; @endphp
                                    <label for="slug">{{ trans("mail::mail.labels.slug") }} *</label>
                                    {{ normalText("mail[slug]","", $errors,null,['hide_label' => true ,"id" => "mail-slug", "class" => "form-control slug required".$className, "placeholder" => trans("mail::mail.labels.slug"),"data-slug" => "target"])}}
                                </div>
                            </div>
                        </div>

                        {{ normalText("mail[subject]","mail::mail.labels.subject", $errors,null,["class" => "form-control required"])}}

                        {{ normalTextarea("mail[body]","mail::mail.labels.body", $errors,null,["class" => "form-control required formated-textarea", "required" => "true"])}}

                        <div class="row">
                            <div class="col-md-6">
                                
                                {{ normalText("mail[cc]","mail::mail.labels.cc", $errors,null,["class" => "form-control"])}}
                            </div>
                            <div class="col-md-6">
        
                                {{ normalText("mail[bcc]","mail::mail.labels.bcc", $errors,null,["class" => "form-control"])}}
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <span data-placement="right" data-toggle="tooltip" title="{!! trans('core::core.options.status.disable') !!}">
                                <label class="switch">
                                    <!--<input type="checkbox" name="mail[status]" class="status">-->

                                    {{ normalCheckbox("mail[status]","", $errors,null,["class" => "status",'grid' => false])}}
