...
@section('title')
{{ trans("menu::menu.titles.edit_menu") }} {{wordWrapper($menu->label, true)}}
@endsection
...
<div class="page-title-header row d-none d-sm-flex">
    <div class="page-header col-sm-12 d-flex pb-4 pt-2">
        <div class="col-sm-6">
            <h4 class="page-title">{{ trans("menu::menu.titles.edit_menu") }} {{wordWrapper($menu->label, true)}}</h4>
        </div>
        <div class="col-sm-6">
            <div class="float-right">
                ...
                <button class="btn btn-danger btn-fw" data-form-id="main_form" data-toggle="modal" data-target="#modal-delete-confirmation" data-action-target="{{ route('admin.menu.delete', updateUrlParams([$menu->id])) }}">{{ trans('core::core.buttons.delete') }}</button>
...
<!-- Main content -->
<!-- form start -->
{{ formStart(null,"PUT" ,'admin.menu.update' ,updateUrlParams([$menu->id]), ['id' => 'main_form','enctype'=>'multipart/form-data'])}}
            ...
            <div class="card-body">
                <div class="tab-content" id="custom-tabs-three-tabContent">
                    <div class="tab-pane fade show active" id="custom-tabs-three-menu-info" menu="tabpanel" aria-labelledby="custom-tabs-three-menu-info-tab">
                        {{ normalText("menu[label]","menu::menu.labels.label", $errors,$menu->label,["class" => "form-control required"])}}
                        <div>
                        ...
                        <div class="text-link hide">
                            ...
                            {{ normalText("menu[text_link]","menu::menu.labels.link", $errors,$menu->link,["class" => "form-control"])}}
                        </div>
                        @if(isset($permissions) && !empty($permissions))
                        <div class="form-group dropdown-link">
                            <label for="menu.link">{{trans("menu::menu.labels.link")}}</label>
                            <select name="menu[dropdown_link]" class="select-2 form-control" id="menu-link">
                                    ...
                                    @foreach($modules as $module => $modulePermission)
                                        ...
                                        @foreach($modulePermission as $value => $label)
                                            @if((in_array('index', explode('.', $value)) || in_array('create', explode('.', $value))))
                                                @php
                                                    $selected = $menu->link == $value ? 'selected' : '';
                                                @endphp
                                                <option value="{{$value}}" {{$selected}}>{{trans($label)}}</option>
                                            @endif
                                        @endforeach
                                    @endforeach
                                @endforeach
                        ...
                        <label class="switch">
                            <input type="checkbox" value="{{config('core.enabled')}}" name="menu[link_target]" {{ ($menu->link_target == config("core.enabled")) ? "checked" : ""}}>
                            <span class="slider round"></span>
                        </label>
                        {{ normalText("menu[css_class]","menu::menu.labels.css_class", $errors,$menu->css_class,["class" => "form-control"])}}
                        {{ normalText("menu[icon]","menu::menu.labels.icon", $errors,$menu->icon,["class" => "form-control"])}}
                        {{ normalInputOfType("number","menu[sort_order]", 'banner::banner.labels.sort_order',$errors,$menu->sort_order,["class" => "form-control ", "min"=>"0" ])}}
                        <div>
                            <label for="menu.is_system">{{trans("menu::menu.labels.is_system")}}</label>
                        </div>
                        <label class="switch">
                            <input type="checkbox" value="{{config('core.enabled')}}" name="menu[is_system]" {{ ($menu->is_system == config("core.enabled")) ? "checked" : ""}}>
                            <span class="slider round"></span>
                        </label>
                        <div>
                        ...
                        <label class="switch">
                            <input type="checkbox" value="{{config('core.enabled')}}" name="menu[status]" {{ ($menu->status == config("core.enabled")) ? "checked" : ""}}>
                            <span class="slider round"></span>
                        </label>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        if (menuLink !== undefined && menuLink != "") {
            if (menuLink.includes('http') || menuLink.includes('https')) {
                jQuery('input.custom-link').prop('checked', true);
