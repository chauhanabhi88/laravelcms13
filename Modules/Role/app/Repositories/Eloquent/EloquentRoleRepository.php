        ...
        if (! empty($user)) {
            if (! empty($user->role)) {
                if ($user->role->slug != config('user.master_admin_slug')) {
                    $masterAdminRole = $this->findBySlug(config('user.master_admin_slug'));
                    $roleId = $masterAdminRole ? $masterAdminRole->id : null;
                }
            }
        }
    ...
    public function getModulePermissions()
    {
        ...
        foreach ($moduleList as $module) {
            ...
            if (file_exists($path.'/config/permissions.php')) {
                $permissions[] = include $path.'/config/permissions.php';
            }
        }
