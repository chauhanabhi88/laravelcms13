...
@section('title')
{{ trans("role::role.titles.edit_role") }} {{wordWrapper($role->name, true)}}
@endsection
...
<div class="page-title-header row d-none d-sm-flex">
    <div class="page-header col-sm-12 d-flex pb-4 pt-2">
        <div class="col-sm-6">
            <h4 class="page-title">{{ trans("role::role.titles.edit_role") }} {{wordWrapper($role->name,true)}}</h4>
        </div>
        <div class="col-sm-6 btn-right">
            <div class="float-right">
                ...
                <button class="btn btn-danger btn-fw" data-form-id="main_form" data-toggle="modal" data-target="#modal-delete-confirmation" data-action-target="{{ route('admin.role.delete', updateUrlParams([$role->id])) }}">{{ trans('core::core.buttons.delete') }}</button>
                @endif
...
{{ formStart(null,"PUT" ,'admin.role.update',updateUrlParams([$role->id]), ['id' => 'main_form','enctype'=>'multipart/form-data'])}}
            ...
            <div class="card-body">
                <div class="tab-content" id="custom-tabs-three-tabContent">
                    <div class="tab-pane fade show active" id="custom-tabs-three-role-info" role="tabpanel" aria-labelledby="custom-tabs-three-role-info-tab">
                        {{ normalText("role[name]","role::role.labels.name",$errors, $role->name,["class" => "form-control required", "data-slug" => $role->slug == config('role.master_admin_slug') ? '' : 'source'])}}
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group mb-0">
                                ...
                                {{ normalText("role[slug]","",$errors, $role->slug,["id" => "role-slug", "class" => "form-control slug ".$className, "placeholder" => trans("role::role.labels.slug"), "data-slug" => "target", "readonly"=>"readonly","form_div"=>false])}}
                            </div>
                    ...
                    <div class="tab-pane fade" id="custom-tabs-three-permissions" role="tabpanel" aria-labelledby="custom-tabs-three-permissions-tab">
                        ...
                        <div class="accordion md-accordion" id="accordionEx_{{ $i }}" role="tablist" aria-multiselectable="true">
                            <div class="card">
                                <div class="card-header" role="tab" id="heading_{{ $i }}" data-toggle="collapse" data-target="#collapse_{{ $i }}" aria-expanded="false">
                                    <a class="btn-tool">
                                ...
                                <div id="collapse_{{ $i }}" class="collapse show" role="tabpanel" aria-labelledby="heading_{{ $i }}" data-parent="#accordionEx_{{ $i }}">
                                    <div class="card-body table-responsive">
                                        <div class="col-md-12 filter-main">
                                            ...
                                            @php
                                            $isChecked = (in_array($value, $rolePermissions)) ? true : false;
                                            @endphp
                                            <div class="form-group row">
                                                <label class="col-sm-3 col-form-label">{{ trans($label) }}</label>
                                                <div class="col-sm-9">
                                                    
                                                    {{ normalCheckbox("permissions[]"," ", $errors,$value,['checked' => $isChecked,'data-bootstrap-switch' => true, 'data-on-color' => 'info', 'data-on-text' => trans("role::role.labels.allow"), 'data-off-text' => trans("role::role.labels.deny"), 'class' => 'permission_switch '.$moduleSlug, 'data-module' => $moduleSlug,'hide_label'=>true])}}
                                                </div>
                                            </div>
                                            @endforeach
                                        </div>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery("#custom-tabs-three-permissions-tab").on('shown.bs.tab', function(e) {
            setTimeout(function() {
                jQuery(".permission_switch").each(function() {
