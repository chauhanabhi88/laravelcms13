...
class CronController extends BackendController
{
     ...
     * @return Response
     */
    public function create(CronRepository $cron)
    {
        try {
    ...
    public function runCron(Request $request)
    {
        try {
            ...
            // Only allow commands registered as cron entries, never arbitrary Artisan commands from the URL.
            $registeredCron = $this->cron->getByAttributes(['command' => $command]);
            if (count($registeredCron) === 0) {
                throw new \Exception(trans('cron::cron.messages.data_invalid'));
            }
