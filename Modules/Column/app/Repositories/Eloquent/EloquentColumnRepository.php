...
class EloquentColumnRepository extends EloquentBaseRepository implements ColumnRepository
{
    ...
    public function getFilters($request, $yesNoOptions, $menuOptions)
    {
        $fields = [
            ...
            [
                ...
                'row' => '2',
                'name' => 'code',
                'value' => $request->get('code', getSessionFilter(config('column.cache.name'), 'code')),
            ...
            [
                ...
                'row' => '2',
                'name' => 'is_sortable',
                'value' => $request->get('is_sortable', getSessionFilter(config('column.cache.name'), 'is_sortable')),
            ...
            [
                ...
                'row' => '2',
                'name' => 'is_default',
                'value' => $request->get('is_default', getSessionFilter(config('column.cache.name'), 'is_default')),
                'select_options' => $yesNoOptions,
            ...
            [
                ...
                'row' => '2',
                'name' => 'menu',
                'value' => $request->get('menu', getSessionFilter(config('column.cache.name'), 'menu')),
    ...
    public function filter($request)
    {
        ...
        $whereCond = $request->get('code', getSessionFilter(config('column.cache.name'), 'code'));
        if ($whereCond !== null) {
            $collection->where('code', 'LIKE', '%'.$whereCond.'%');
        }

        $whereCond = $request->get('is_sortable', getSessionFilter(config('column.cache.name'), 'is_sortable'));
        if ($whereCond !== null) {
            $collection->where('is_sortable', $whereCond);
        }
        $whereCond = $request->get('is_default', getSessionFilter(config('column.cache.name'), 'is_default'));
        if ($whereCond !== null) {
            $collection->where('is_default', $whereCond);
        }

        $whereCond = $request->get('menu', getSessionFilter(config('column.cache.name'), 'menu'));
        if ($whereCond !== null) {
