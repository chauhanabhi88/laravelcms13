    ...
    function getImageUrl($param)
    {
      ...
      $param = [
        ...
        */
        ...
        if ((!isset($param['image'])) || (!isset($param['module']))) {
        ...
        if (!empty($param['image-type'])) {
            if ($param['image-type'] == 'resize') {
                if (isset($param['image-size'])) {
                    ...
                    $path  .= $dimensionPath;
                } else if (isset($param['height']) || isset($param['width'])) {
            ...
            } else if ($param['image-type'] == 'thumbnail') {
...
if (!function_exists('imageResize')) {
    ...
    {
        if (file_exists(public_path() . '/storage/' . $moduleName . '/' . $fileName)) {
            if (!empty($width) && !empty($height)) {
                $sizePath = "/" . $moduleName . '/' . $height . 'x' . $width . "/";
                $folderPath = $height . 'x' . $width . "/";
            } else if (!empty($width)) {
                $sizePath = "/" . $moduleName . '/w' . $width . "/";
                $folderPath = 'w' . $width . "/";
            } else if (!empty($height)) {
                $sizePath = "/" . $moduleName . '/h' . $height . "/";
                $folderPath = 'h' . $height . "/";
            }
            if ($folderName) {
                $sizePath = "/" . $moduleName . '/' . $folderName . "/";
                $folderPath =  $folderName . "/";
            }
            if (!Storage::disk('public')->exists($sizePath)) {
                \File::makeDirectory(public_path('storage' . $sizePath), 0777, true);
            }
            Storage::put('public' . $sizePath . $fileName, 'content');
            copy(public_path('storage') . '/' . $moduleName . '/' . $fileName, public_path() . '/storage' . $sizePath . $fileName);

            $manager = new ImageManager(new Driver());
            $img = $manager->read(public_path() . '/storage' . $sizePath . $fileName);
            /*$image = $image->resize($this->_uploadParams['thumbnail_size'], null, function ($constraint) {
                $constraint->aspectRatio();
            });
            $image->save();*/

            //$img = \Image::make(public_path() . '/storage' . $sizePath . $fileName);
            if ($crop && !empty($width) && !empty($height)) {
                $img->crop($width, $height, $xCoordinate, $yCoordinate)->save();
                return $folderPath;
            } else if ($crop && !empty($width)) {
                $img->orientate()
                    ->fit($width, $height, function ($constraint) use ($upsize) {
                        if ($upsize) {
                            $constraint->upsize();
                        }
                    })->save();
...
if (!function_exists('getImageByParticularDimension_new')) {
    ...
    function getImageByParticularDimension_new($file, $module, $width = '100', $height = '100', $folderName = null)
    {
        $sizePath = "/" . $module . '/' . $height . 'x' . $width . "/";
        if ($folderName) {
            $sizePath = "/" . $module . '/' . $folderName . "/";
        }
        if (!Storage::disk('public')->exists($sizePath)) {
            \File::makeDirectory(public_path('storage' . $sizePath), 0777, true);
        }
        Storage::put('public' . $sizePath . $file, 'content');
        copy(public_path('storage') . '/' . $module . '/' . $file, public_path() . '/storage' . $sizePath . $file);

        $resizeImg = \Image::make(public_path() . '/storage' . $sizePath . $file)->resize($width, $height, function ($constraint) {
            $constraint->aspectRatio();
        })->save();
    }
}
...
if (!function_exists('encrypt_It')) {
    function encrypt_It($plaintext)
    {
        ...
        $iv = \Config::get('core.encrypt.iv');
        $ciphertext =  base64_encode(openssl_encrypt($plaintext, $cipher, $password, $options = 0, $iv));
        return $ciphertext;
    }
}
/**
...
if (!function_exists('decrypt_It')) {
    function decrypt_It($encrypted)
    {
        ...
        $password = \Config::get('core.encrypt.password');
        $iv = \Config::get('core.encrypt.iv');
        $decrypted = openssl_decrypt(base64_decode($encrypted), $cipher, $password, $options = 0, $iv);
        return $decrypted;
    }
}
...
if (!function_exists('wordWrapper')) {
    function wordWrapper($str, $quote = false, $length = 50, $symbol = '...', $wordbreak = false)
    {
        ...
        if (!empty($str) && strlen($str) >= $length) {
            ...
            return false;
            exit();
        } else if (!empty($str) && strlen($str) < $length) {
            if ($quote) {
                echo '"' . strip_tags($str) . '"';
                return false;
                exit();
...
if (!function_exists('setFilterSession')) {
    function setFilterSession($moduleName, $request, $key = null, $sessionKey = null)
    {
        ...
        } else if (!empty($key) && isset($moduleName) && !empty($moduleName) && isset($request) && !empty($request) && !is_object($request)) {
...
if (!function_exists('getColumnObject')) {
    ...
    {
        return New Modules\Column\Models\Column();
    }
}
...
if (!function_exists('getActiveMenuId')) {
    ...
    {
        $activeMenuId = ($request instanceof \Illuminate\Http\Request) ? $request->get('active_menu_id') : null;
        ...
        return \Modules\Menu\Models\Menu::where('link', $menuRouteName)->value('id');
    }
}
...
if (!function_exists("changeKeyToIndex")) {
    function changeKeyToIndex($array)
    {
        ...
        foreach ($array as $k => $val) {
            if (is_array($val)) $array[$k] = changeKeyToIndex($val); //recurse
