...
class ModuleRepository
{
    ...
    public function update($params)
    {
        ...
        // Goes through the shared trait rather than reaching for the store's own
        // flush methods through the cache repository: those only exist on
        // Modules\Core's FileStore, so on any other driver this threw a
        // BadMethodCallException.
        if (isset($params['clear_cache'])) {
            $keys = config(strtolower($moduleName).'.cache');

            foreach (! empty($keys) ? $keys : [config($module->getLowerName().'.name')] as $key) {
                $this->flushCacheFor($key);
            }
        } elseif (! getModule('core', 'cache')) {
            $this->flushCacheFor(config('core.name'));
        }
    }
    ...
    {
        $data = [];
        // Schema::getTables() replaces "SHOW TABLES", which only works on MySQL
        // and needed the Tables_in_<database> column name reconstructed by hand.
