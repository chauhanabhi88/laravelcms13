...
abstract class EloquentBaseRepository implements BaseRepository
{
   ...
   public function destroyMultiple($request, $removeFile = false, $notDeleteIds = [])
    ...
    {
        // Goes through the shared guard: calling modulePath() directly threw a
        // BadMethodCallException on any store other than the module FileStore.
    ...
    private function _setExportHeader($start, $end, $objSpreadsheet, $val = array())
    {
        $count = 0;
        $styleArray = [
            'font' => [
    ...
    private function _setExportData($start, $end, $val = array(), $objSpreadsheet, $rowCount, $column)
    {
        $count = 0;
        for ($i = $start; $i <= $end; $i++) {
            /*if ($rowCount % 2 == 0) {
