...
class EloquentBlockRepository extends EloquentBaseRepository implements BlockRepository
{
    public function sortColumns($request)
    {
        $columns = [
            ...
            [
                "title" => trans("block::block.titles.title"),
                "column" => "title"
            ],
            // [
            //     "title" => trans("block::block.titles.is_enabled"),
            //     "column" => "is_enabled"
            // ],
            [
        ...
        if ($this->getAuthUser()->can("admin.block.mass_delete")) {
            $massDeleteCheckbox = [
                "column" => "massDelete",
                "type" => "massDelete",
            ];
    ...
    public function filter($request)
    {
        ...
        $whereCond = $request->get('content', getSessionFilter(config("block.name"), "content"));
        if ($whereCond !== null) {
            $content = $whereCond;
            $collection->where("content", "LIKE", "%{$content}%");
        }
