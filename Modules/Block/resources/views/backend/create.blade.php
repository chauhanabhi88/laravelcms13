...
@section('title')
{{ trans("block::block.titles.create_block") }}
@endsection
...
<div class="page-title-header row d-none d-sm-flex">
  <div class="page-header col-sm-12 d-flex pb-4 pt-2">
    <div class="col-sm-6">
      <h4 class="page-title">{{ trans("block::block.titles.create_block") }}</h4>
    </div>
...
{{ formStart(null,"POST" ,'admin.block.store' ,updateUrlParams(), ['id' => 'main_form','enctype'=>'multipart/form-data'])}}
      ...
      <div class="card-body">
        <div class="tab-content">
            ...
            <div class="card-body">
              <div class="tab-content">
                ...
                <div class="tab-pane {{ App::getLocale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                  @include('block::backend.partials.create-translatable-fields', ['lang' => $locale])
                </div>
                @endforeach
              </div>
            </div>
          ...
          <div class="tab-pane ctab-pane {{isset($languageOptions) && (count($languageOptions)) > 1 ? '' : 'active'}}" id="general-form">
            ...
            @endforeach
            @endif
            @include('block::backend.partials.create-fields')
          </div>
        </div>
      </div>
    </div>
