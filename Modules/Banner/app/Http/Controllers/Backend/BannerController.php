...
class BannerController extends BackendController
{
    ...
    public function create(Request $request)
    {
        try {
            ...
            $countryCodes = [ucfirst(config('banner.country_code')), strtolower(config('banner.country_code')), strtoupper(config('banner.country_code'))];
            $countryCode = config('banner.country_code');
            $countryCodeU = ucfirst(config('banner.country_code'));
            $statusOptions = $this->banner->getStatusOptions(true);
            $bannerGroups = $this->bannerGroup->getBannerGroups(true);
            $uploadLimit = settings('banner', 'max_upload_size');
            $imageTypes = $this->getImageTypes();

            return view('banner::backend.banner.create', compact('statusOptions', 'bannerGroups', 'imageTypes', 'uploadLimit', 'languageOptions', 'countryCode', 'countryCodeU', 'countryCodes'));
        } catch (\Throwable $e) {
            return redirect()->route('admin.dashboard.index',updateUrlParams())->with("error", $e->getMessage());
        }
    }
    ...
    public function edit(Request $request)
    {
        try {
            ...
            $uploadLimit = settings('banner', 'max_upload_size');
            $imageTypes = $this->getImageTypes();
            return view('banner::backend.banner.edit', compact("banner", "statusOptions", "bannerGroups", 'uploadLimit', 'imageTypes', 'languageOptions', 'countryCodes'));
        } catch (\Throwable $e) {
