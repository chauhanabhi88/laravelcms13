...
@section('title')
{{ trans("banner::banner.titles.edit_banner")}} {{wordWrapper($banner->title, true)}}
@endsection
...
<div class="page-title-header row d-none d-sm-flex">
  <div class="page-header col-sm-12 d-flex pb-4 pt-2">
    <div class="col-sm-6">
      <h4 class="page-title">{{ trans("banner::banner.titles.edit_banner")}} {{wordWrapper($banner->title, true)}}</h4>
    </div>
    <div class="col-sm-6 btn-right">
      <div class="float-right">
        ...
        <button class="btn btn-danger btn-fw" data-form-id="main_form" data-toggle="modal" data-target="#modal-delete-confirmation" data-action-target="{{ route('admin.banner.delete', updateUrlParams([$banner->id])) }}">{{ trans('core::core.buttons.delete') }}</button>
        @endcan
...
{{ formStart(null,"PUT" ,'admin.banner.update' ,updateUrlParams([$banner->id]), ['id' => 'main_form','enctype'=>'multipart/form-data'])}}
      ...
      <div class="card-body">
        <div class="tab-content">
            ...
            <div class="card-body">
              <div class="tab-content">
                ...
                <div class="tab-pane {{ App::getLocale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                  @include('banner::backend.banner.partials.edit-translatable-fields', ['lang' => $locale])
                </div>
                @endforeach
              </div>
            </div>
          ...
          <div class="tab-pane ctab-pane {{isset($languageOptions) && (count($languageOptions)) > 1 ? '' : 'active'}}" id="general-form">
            ...
            @endforeach
            @endif
            @include('banner::backend.banner.partials.edit-fields')
          </div>
        </div>
      </div>
    </div>
